table 96100 "Incident Assets Real Estate"
{
    ...
    fields
    {
        ...
        field(2; "Fixed Real Estate No."; Code[20])
        {
            ...
            trigger OnValidate()
            var
                fixedRERec: Record "Fixed Real Estate"; 
            begin   
                if not fixedRERec.Get(rec."Fixed Real Estate No.") then
                    fixedRERec.init;
                rec."REF Description" := fixedRERec.Description;    
            end;
        }
        field(3; "Contract No."; Code[20])
        {
            ...
            trigger OnValidate()
            var
                LeaseContractRec: Record "Lease Contract";              
            begin   
                if not LeaseContractRec.Get(rec."Contract No.") then
                    LeaseContractRec.init;  
                Validate("Contact No", LeaseContractRec."Contact No.");
            end;
        }
